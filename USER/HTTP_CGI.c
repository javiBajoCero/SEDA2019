...
//  led_2 = 0;
  ...
  do {
    ...
    if (var[0] != 0) {
      ...
      if (NULL!=(COM_ptr=( char *)strstr (var,(const char *)  "comandos="))) {
				for(o=0;o<sizeof(comandos);o++){//resetea a 0 el array
					comandos[o]=0x00;
				}
				strcpy(comandos,(COM_ptr+strlen("comandos=")));
				strcat(comandos,"\r"); 	
				
      }
      else if (str_scomp (var, "leds=OFF") == __TRUE) {
         leds = 1;
         led_1 = 0;
         led_2 = 0;
      }
      else if (str_scomp (var, "led_1=ON") == __TRUE) {
         if (!leds) led_1 = 1;

      }
      else if (str_scomp (var, "led_2=ON") == __TRUE) {
         if (!leds) led_2 = 1;
